...
class SCTextField extends TextField implements ReadOnlyable, ShowOnlyable, RightClickable {
	...
	SCTextField(final Resource res, String text, boolean limited, int maxLength) {
		...
		addEventFilter(MouseEvent.ANY, new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				if (controlType == ControlType.SHOWONLY) {
					...
					Client.ControlButtonEvent(res.getId(), SCTextField.this, event);
					event.consume();
				}
			}
		});
		...
		focusedProperty().addListener(new ChangeListener<Boolean> () {
			@Override public void changed(ObservableValue<? extends Boolean> observable,
					Boolean oldValue, Boolean newValue) {
				if (!newValue)
					currentCaretPosition = caretPositionProperty().intValue();
			}
		});
	}
    ...
    /**
     * Looks like we need this when doing a ctrl-V into the edit box when 
     * something is selected.
     * Needs work.
     * Should replace characters up to maxlength
     * 
     * @param text
     */
    @Override
    public void replaceSelection(String text) {
    	if (!programmatic &&
    			(controlType == ControlType.SHOWONLY ||controlType == ControlType.READONLY)) return; 
	...
	/**
	 * Note: No need to suppress the ITEM message sent from code in PanelDialogControlFactory
	 * DX Sends the ITEM message in this case even though it is a 
	 * programmatic change
	 * @param data New text
	 */
